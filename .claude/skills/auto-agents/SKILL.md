---
name: auto-agents
description: 화 집필/수정 파이프라인 오케스트레이션. Mode A(초고 Sub-agents 7개), Mode B(수정 Agent Teams). 파이프라인, 집필 시작, 수정 파이프라인 시 사용.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash, Task
---

# Auto-Agents - 화 집필/수정 오케스트레이션 스킬

검증된 에이전트 패턴을 통합한 재사용 워크플로우.
두 가지 모드를 제공: **Mode A (Draft)** = 초고 집필, **Mode B (Revision)** = 수정/피드백 반영.

## Trigger
- "파이프라인", "집필 시작", "수정 파이프라인", "auto-agents", "화 집필", "수정 시작"

## Mode Selection

사용자에게 모드 확인:
- **Mode A (Draft)**: 새 화 초고 집필 — Sub-agents 8개
- **Mode B (Revision)**: 기존 화 수정/피드백 반영 — Agent Teams + Sub-agents
...
1. `reference/ability-upgrade-current.md` 마지막 20줄만 읽기 → 마지막 소(小) 업그레이드 화 확인
2. `(현재 화) - (마지막 소 업그레이드 화)` 계산: 4화 초과=필수, 3-4화=권장, 1-2화=선택
3. 같은 계통 3회 연속 여부 확인 → 해당 시 다른 계통으로 강제 전환
...
**Tier 2: 캐릭터 (~20-40KB — 해당 화 등장 캐릭터만)**
4. 등장 캐릭터 파일 + 감정추적 파일 로드 (비등장 캐릭터 로드 금지)

**Tier 3: 플롯 (~15KB)**
5. `03-플롯/메인플롯-상세/PartN.md` 해당 화 섹션만
6. `03-플롯/서사줄기추적.md` 최근 5화만
7. `03-플롯/복선추적.md` active 복선만 + `03-플롯/쾌감카운터.md` (E패턴 연속/저사용)
8. 직전 2ep 패턴 유형 확인 — 화 성격 3연속 방지
9. `01-세계관/타임라인.md`, `01-세계관/타워배경.md` 확인
10. 능력 합성 기회 현황 확인
...
13. **행위소/복선/합성매핑 필요 시만**: `reference/plot-structure.md` — 화 성격·클라이맥스는 `core-rules.md` §13으로 자동주입됨

#### 선언 (매 화)
14. **화 성격 유형 선언** — 포석형/돌파형/여운형/인간형/전환형 중 하나
15. **씬 클라이맥스 위치 선언** — 전면배치/중간배치/후면배치 중 하나
16. **엔딩 방식 확인** — 직전 화와 동일 방식 금지
...
3. 저장: `04-원고/[권]/챕터/ep-[NNN]_outline.md`
4. 포함 항목:
   - 화 개요 (핵심 사건, 변화, 독자 경험)
   ...
   - 화 성격 유형, 씬 클라이맥스 위치, 엔딩 방식 명시
   - 능력 업그레이드 배치 여부 + 계통/레퍼토리 코드
   - 전투씬 포함 시: 전투 스타일 선언, 규모, 적 분석
...
- 아웃라인 해당 에피소드 부분, 캐릭터 상태 요약, 이음새, 화 메타 포함
...
- 글자수 목표: ~2,000-3,500자 (화 ~6,000-7,000자의 1/3~1/2)
- 전투씬 포함 시: 능력 쇼케이스 원칙 준수 (상성간파→능력발동→압도→경악→페널티유머)
- 톤 파괴어 절대 사용 금지
...
3. 씬 전환 마커 (`* * *`) 삽입
4. 전체 읽기 흐름 확인
5. 저장: `04-원고/[권]/챕터/ep-[NNN]_v1_초고.md`
...
| 스캔 | Grep 패턴 | 대응 |
|------|----------|------|
| 톤 파괴어 | `스킬트리\|빌드\|너프\|버프\|쿨타임\|어그로\|딜러\|탱커\|힐러\|파밍` | 세계관에 맞는 표현으로 즉시 대체 |
...
- **world-building.md는 타워/능력 검증 필요 시만 Read**
- Phase 1 컨텍스트 요약을 우선 사용. 직접 Read는 타임라인·타워배경에 한정
- 대상: 초고 + 관련 설정 파일
- 점수 산출 (0-100) + Critical/Major/Minor 분류
- **7화 초과 시 분할 필수** (L12): R2a (일관성) + R2b (서사구조)로 분리. 각각 별도 Sub-agent로 실행
- **일관성 검사 항목** (R2a — 분할 시):
  - 복선 상태 감사
  - 타워 설정 정합성 + 능력 등급 정합성 검증
  - 합성 매핑 정확도 검증
- **서사구조 검사 항목** (R2b — 분할 시):
  - 쾌감 패턴 배치 적절성 (E패턴 연속 2회 금지, 저사용 패턴 점검)
  - 서사줄기 비율 확인 (타워공략/인간관계/정체성)
  - 화 성격 유형 정합성 검증
  - 페이싱 평가
- **전투씬 포함 시**: 전투 스타일 선언 존재 여부, 능력 등급 정합성, 개인 전투 교환 수, 능력 쇼케이스 원칙 준수
...
2. **우선순위 규칙 적용**:
   - 세계관/타워 설정 정합성 (R2) > 인간화 (R1): 설정 오류는 문체보다 우선
...
3. 수정 유형별 처리:
   ...
   - 전투씬 → 능력 쇼케이스/합성 묘사/페널티 유머 보완

**퇴고 체크리스트** (동시 적용):
4. em-dash(—) 화당 5회 이하 확인
5. 메타 서술 라벨 + 독자 설명 라벨 + 중복 설명 제거
6. 에피소드 종결 패턴 — 직전 화와 동일 방식 금지
...
11. **분량 점검** — 화 총 글자 수 7,000자 이상, 씬별 2,000자 이상
12. 수정 건수 기록 (P1 N건, P2 N건)
13. **회귀 검출 (Regression Detection)**: 이번 화에서 수정한 패턴이 직전 10화에도 존재하는지 Grep 스캔
    - 대상: `ep-[N-10]` ~ `ep-[N-1]` 최신 버전 파일
    - 검출 항목: Phase 6에서 **3건 이상** 수정한 패턴 유형만 (예: 분리호칭 5건 수정 → 직전 10화에서 분리호칭 Grep)
    - **출력**: `[회귀 경고] ch-XXX: 분리호칭 N건, SVO 단문 N건` — Phase 8 보고에 포함
    - **조치 없음**: 자동 수정하지 않음. 다음 Mode B 수정 범위에 포함할 후보로 기록만
14. 퇴고 완료 후 저장: `ep-[NNN]_v2_1차수정.md`
...
- `03-플롯/서사줄기추적.md` — 줄기 비율 + 화 성격 유형 기록
...
- `01-세계관/타임라인.md` 업데이트
- `reference/ability-upgrade-current.md` — 능력 업그레이드 기록 추가 (계통/규모/레퍼토리/4요소 체크)

### Phase 8: Verification [Lead]

최종 검증 및 보고.

1. 화 파일 존재 + 글자수 확인 (6,000~7,000자)
2. 추적 파일 업데이트 확인 (양쪽 Updater 결과)
3. 리뷰 점수 종합 보고:
   - Phase 4B 기계적 스캔: 톤 파괴어 N건, em-dash N건, 분리호칭 N건 (즉시 수정 완료)
   - R1 인간화 심층: X/100
   - R2 일관성+서사구조: X/100, P1 N건
   - R3 재미평가: X/100 (실행 시)
   - Phase 5.5 충돌: N건 해소 / N건 사용자 판단
4. 능력 업그레이드 기록 확인 (계통/규모/레퍼토리 코드)
5. **회귀 검출 보고**: 직전 10화에서 발견된 잔존 패턴 요약 (해당 시). Mode B 수정 후보 목록으로 기록
6. 다음 화 주요 컨텍스트 요약
...
| 규모 | 화 수 | Teammates | Sub-agents | 총 에이전트 | 기준 |
|------|---------|-----------|------------|------------|------|
| Small | 1-3화 | 2 | 2 | 4 | 조기 진단, 단일 화 수정 |
| Medium | 4-7화 | 2 | 3-5 | 5-7 | 반 Part 수정 |
| Large | 8-15화 | 4 | 5-9 | 9-13 | Full Part 수정 |
...
| Large | editor-a~d | sub-editor-a → sub-editor-b → sub-editor-c → sub-editor-d | **2병렬 순차**: (a∥b) → (c∥d). 겹치지 않는 화 그룹끼리만 병렬 |
...
#### 순차 실행 시 이음새 처리
- Medium/Large에서 앞 그룹 Sub-agent 완료 후, 마지막 화 끝 500자를 다음 그룹 Sub-agent에게 컨텍스트로 전달
- 이음새 편집은 Lead가 Phase 2 종료 후 일괄 처리

#### Teammate 사망 대응 → Sub-agent 재시도
- Sub-agent 실패(타임아웃/오류) 시 동일 프롬프트로 1회 재시도
- 2회 연속 실패 시 해당 화 범위를 Lead가 직접 처리
...
- `editor`: 원고 수정 전담 (전체 화)

#### Medium (2 Teammates)
- `editor-early`: 전반 화 원고 수정
- `editor-late`: 후반 화 원고 수정
...
- 타워 설정 정합성 + 능력 등급 + 쾌감 패턴 + 서사줄기 재검증
...
### Mode A
- 완성된 화 퇴고본 (`ep-[NNN]_v2_1차수정.md`)
- 화 아웃라인 (`ep-[NNN]_outline.md`)
- 리뷰 점수 2~3종 (R1 인간화 / R2 일관성+서사구조 / R3 재미평가(선택))
- Phase 5.5 충돌 해소 로그 (해당 시)
- 업데이트된 추적 파일 목록
- 능력 업그레이드 기록 (해당 시)
- 회귀 검출 보고 (직전 10화 잔존 패턴 — 해당 시)
- 다음 화 컨텍스트 요약

### Mode B
- 수정된 화 파일 (`ep-[NNN]_v[N+1]_[수정단계].md`)
- 수정 건수 총계 + 분류
...
- 글자수 부족 P1 발생 시 Phase 6에서 해당 에피소드 확장 (밀도 있는 확장만 — 내면 묘사/능력 전략/대화 비트/감각 묘사).
